...
class Multitone:
    ...
    def __init__ (self, tones: int, del_freq:float, del_time:float, amp:float, model: str = 'USa1_v0') -> None:
    ...
    def reset(self, model: str, manual:str=None) -> tuple(np.ndarray, dict):
        # アルゴリズム選択
        match model:
            case 'all0':
    ...
    def step(self, action) -> tuple(np.ndarray, float, bool, bool, dict):
        ...
        match self.model:
            ...
            case 'USo_v0':
                ...
                score = (self.tones**2 - max_upper_peaks_array[1]) / (self.tones**2)
            case 'USt_v0':
                observation = max2_upper_peaks_array
                score = ((self.tones**2 - max2_upper_peaks_array[1][0]) + (self.tones**2 - max2_upper_peaks_array[1][1])) / (2 * self.tones**2)
            case 'UFtSt_v0':
                observation = max2_upper_peaks_array
                score = ((self.tones**2 - max2_upper_peaks_array[1][1]) - (max2_upper_peaks_array[1][0] - max2_upper_peaks_array[1][1])) / (self.tones**2)
            case 'BSa1_v0':
                avg_upper_peaks_height = np.average(np.take(ep_t_array, upper_peaks))
                ...
                score = ((self.tones**2 - avg_upper_peaks_height) + avg_lower_peaks_height) / (2 * self.tones**2)
            case 'BSa2_v0':
                avg_upper_peaks_height = np.average(np.take(ep_t_array, upper_peaks))
                avg_lower_peaks_height = np.average(np.take(ep_t_array, lower_peaks))
            ...
            case 'BSo_v0':
                ...
                score = ((self.tones**2 - max_upper_peaks_array[1]) + max_lower_peaks_array[1]) / (2 * self.tones**2)
            case 'BSt_v0':
                observation = max2_both_peaks_array
                score = ((self.tones**2 - max2_upper_peaks_array[1][0]) + (self.tones**2 - max2_upper_peaks_array[1][1]) +\
                          max2_lower_peaks_array[1][0] + max2_lower_peaks_array[1][1]) / (4 * self.tones**2)
            case 'BFt_v0':
                observation = max2_both_peaks_array
                score = ((max2_upper_peaks_array[1][0] - max2_lower_peaks_array[1][0]) + (max2_upper_peaks_array[1][1] - max2_lower_peaks_array[1][1])) / (2 * self.tones**2)

        # terminated
        if (max_ep_t <= self.tones):
            terminated = True
        else:
            terminated = False
