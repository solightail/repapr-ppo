...
class Agent:
    def __init__(self, n_actions, input_dims, gamma=0.99, alpha=0.0003, gae_lambda=0.95, policy_clip=0.2, batch_size=64, N=2048, n_epochs=10):
        ...
        # choose_action 関数内の action 定義において、引数として利用
    ...
    def choose_action(self, observation):
        ...
        dist = self.actor(state)
        value = self.critic(state)
        # dist = ActorNetwork(nn.Module).forward(state) -> Categorical(Distribution)
        # sample_n メソッドは Distribution クラスに記載
        # sample_n(self, n: int) -> torch.Tensor
        action = dist.sample_n(self.n_actions)
